...
import { UpdateSpaRequestModel } from "../../../api/requestModels/UpdateSpaRequestModel"
...
const params = useParams() as { id: string }
	...
	const [loadResult, setLoadResult] = useState<Spa>();
    ...
    const load = async () => {
	            ...
	            const result = await httpGet<Spa>(`/Spa/${params.id}`)
	            if(result.isSuccess) {
...
setname(result.data.name)
	...
	const submit = async () => {
        ...
        const result = await httpPut<UpdateSpaRequestModel>("/Spa", {
        	id: params.id,
            name: name
        } as UpdateSpaRequestModel);
                    ...
                    <Grid container style={{width: "100%", minHeight: "100vh"}} justify="center" alignItems="center">
                        <Grid item xs={10} sm={8} md={6} lg={4} xl={4}>
                            <Card style={{width: "100%", padding: "20px", display: "flex", justifyContent: "center", flexDirection: "column", textAlign: "center"}}> 
                                <Typography style={{paddingBottom: "10px"}} variant="h5">Update Spa</Typography>
                                {success
	                            ? <Alert style={{margin: "10px 0"}} severity="success">
	                                <AlertTitle>Success</AlertTitle>
	                                Spa was updated successfully
	                            </Alert>
	                            : null}
	                            {error || loadError
