...
import { CreateSpaRequestModel } from "../../../api/requestModels/CreateSpaRequestModel";
...
const CreateSpaPage = () => {
	...
	const submit = async () => {
        ...
        const result = await httpPost<CreateSpaRequestModel>("/Spa", {
            name: name
        } as CreateSpaRequestModel);
                    ...
                    <Grid container style={{width: "100%", minHeight: "100vh"}} justify="center" alignItems="center">
                        <Grid item xs={10} sm={8} md={6} lg={4} xl={4}>
                            <Card style={{width: "100%", padding: "20px", display: "flex", justifyContent: "center", flexDirection: "column", textAlign: "center"}}> 
                                <Typography style={{paddingBottom: "10px"}} variant="h5">Create Spa</Typography>
                                {success
	                            ? <Alert style={{margin: "10px 0"}} severity="success">
	                                <AlertTitle>Success</AlertTitle>
	                                Spa was created successfully
	                            </Alert>
	                            : null}
	                            {error || loadError
