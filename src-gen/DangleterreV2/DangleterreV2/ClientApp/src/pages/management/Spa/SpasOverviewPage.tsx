...
import { Spa } from "../../../api/models/entities/Spa";
import { Alert, AlertTitle } from "@material-ui/lab";

const SpasOverviewPage = () => {

		const history = useHistory()

    	const [loading, setLoading] = useState(false);
        const [error, setError] = useState<String>();
        const [SpaResult, setSpaResult] = useState<Spa[]>([])
        ...
        const fetchData = async () => {
            ...
            setLoading(true);
            var result = await httpGet<Spa[]>("/Spa")
    
            if(result.isSuccess) {
                setSpaResult(result.data);
            } else {
        ...
        const deleteEntity = async (id: String) => {
	                ...
	                var result = await httpDelete<boolean>(`/Spa?id=${id}`)
	                
	                if(result.isSuccess) {
                    	SpaResult.splice(SpaResult.indexOf(SpaResult.filter(e => e.id === id)[0]),1)
                    	setSpaResult([...SpaResult]);
	                } 
                    ...
                    <Grid container justify="center">
                        <Grid item xs={12} sm={12} md={12} lg={12} xl={12} style={{padding: "20px"}}>
                            <div style={{width: "100%", display: "flex", justifyContent: "center", flexDirection: "column"}}>
                                <Typography variant="h4">Spa overview</Typography>
                                <Card>
...
<TableCell>{row.name}</TableCell>
                                                    <TableCell>
                                                    	<Button color="primary" variant="outlined" onClick={() => history.push(`/management/Spa_update/${row.id}`)}>Edit</Button>
                                                    </TableCell>
                                            		<TableCell>
...
export default SpasOverviewPage;
