...
const RoomSchedulesOverviewPage = () => {

		const history = useHistory()

    	const [loading, setLoading] = useState(false);
        const [error, setError] = useState<String>();
        const [RoomScheduleResult, setRoomScheduleResult] = useState<RoomSchedule[]>([])
        ...
        const fetchData = async () => {
            ...
            if(result.isSuccess) {
                setRoomScheduleResult(result.data);
            } else {
        ...
        const deleteEntity = async (id: String) => {
	                ...
	                var result = await httpDelete<boolean>(`/RoomSchedule?id=${id}`)
	                
	                if(result.isSuccess) {
                    	RoomScheduleResult.splice(RoomScheduleResult.indexOf(RoomScheduleResult.filter(e => e.id === id)[0]),1)
                    	setRoomScheduleResult([...RoomScheduleResult]);
	                } 
                    ...
                    <Grid container justify="center">
                        <Grid item xs={12} sm={12} md={12} lg={12} xl={12} style={{padding: "20px"}}>
                            <div style={{width: "100%", display: "flex", justifyContent: "center", flexDirection: "column"}}>
                                <Typography variant="h4">RoomSchedule overview</Typography>
                                <Card>
...
<TableCell align="right">end</TableCell>
                                            ...
                                            <TableBody>
                                            {RoomScheduleResult.map((row) => (
                                                <TableRow key={row.id}>
<TableCell>{row.name}</TableCell>
<TableCell align="right">{row.start}</TableCell>
<TableCell align="right">{row.end}</TableCell>
                                                    <TableCell>
                                                    	<Button color="primary" variant="outlined" onClick={() => history.push(`/management/RoomSchedule_update/${row.id}`)}>Edit</Button>
                                                    </TableCell>
                                            		<TableCell>
...
export default RoomSchedulesOverviewPage;
