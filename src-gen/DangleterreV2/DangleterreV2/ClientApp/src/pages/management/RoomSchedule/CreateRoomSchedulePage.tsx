...
import { CreateRoomScheduleRequestModel } from "../../../api/requestModels/CreateRoomScheduleRequestModel";


const CreateRoomSchedulePage = () => {
	...
	const submit = async () => {
        ...
        const result = await httpPost<CreateRoomScheduleRequestModel>("/RoomSchedule", {
            name: name, start: start, end: end
        } as CreateRoomScheduleRequestModel);
                    ...
                    <Grid container style={{width: "100%", minHeight: "100vh"}} justify="center" alignItems="center">
                        <Grid item xs={10} sm={8} md={6} lg={4} xl={4}>
                            <Card style={{width: "100%", padding: "20px", display: "flex", justifyContent: "center", flexDirection: "column", textAlign: "center"}}> 
                                <Typography style={{paddingBottom: "10px"}} variant="h5">Create RoomSchedule</Typography>
                                {success
	                            ? <Alert style={{margin: "10px 0"}} severity="success">
	                                <AlertTitle>Success</AlertTitle>
	                                RoomSchedule was created successfully
	                            </Alert>
	                            : null}
	                            {error || loadError
...
export default CreateRoomSchedulePage;
